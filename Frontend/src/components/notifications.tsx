        ...
        <div className={`relative text-center text-elanco ${side ? "text-4xl":"text-4xl md:text-5xl lg:text-6xl"} font-bold p-3`}>
            ...
            <div className={`${side ? "hidden": "absolute"} top-2 right-2 cursor-pointer`} id="hideNoti">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" className="size-8 md:size-10">
                    <path fill="currentColor" fillRule="evenodd" stroke="currentColor" strokeLinejoin="round" strokeWidth="4" d="M44 40.836c-4.893-5.973-9.238-9.362-13.036-10.168c-3.797-.805-7.412-.927-10.846-.365V41L4 23.545L20.118 7v10.167c6.349.05 11.746 2.328 16.192 6.833c4.445 4.505 7.009 10.117 7.69 16.836Z" clipRule="evenodd" />
                </svg>
            </div>
        </div>
        {notifications.map((noti, index) => (
        <div key={index} className={`border-y-2 p-3 ${noti.read ? "opacity-60": "cursor-pointer"}`} onClick={() => markRead(index)}>
